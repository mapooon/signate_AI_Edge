...
def train_model(model,optimizer,dataloaders, num_epochs=25):
    ...
    iou_dict={x:np.array([np.nan]*num_epochs*4).reshape(num_epochs,4) for x in ['train', "validation"]}
    for epoch in range(num_epochs):
        ...
        for phase in ['train', "validation"]:
            ...
            for step,data in enumerate(tqdm(dataloaders[phase])):#tqdm(dataloaders[phase]):
                ...
                running_iou += iou(outputs,labels,average=False)#.item()#*batch_size
            ...
            print('{} Loss: {:.4f} IOU: {:.4f}'.format(phase, epoch_loss, epoch_iou.mean()))
            ...
            if phase=="validation":
                ...
                if epoch>0:
                    ...
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["train"][:epoch+1].mean(1),update="replace",win="iou",name="train")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["train"][:epoch+1,0],update="replace",win="car",name="train")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["train"][:epoch+1,1],update="replace",win="signal",name="train")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["train"][:epoch+1,2],update="replace",win="pedestrian",name="train")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["train"][:epoch+1,3],update="replace",win="lane",name="train")
                    viz.line(X=np.arange(epoch+1),Y=loss_dict["validation"][:epoch+1],update="replace",win="loss",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1].mean(1),update="replace",win="iou",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,0],update="replace",win="car",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,1],update="replace",win="signal",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,2],update="replace",win="pedestrian",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,3],update="replace",win="lane",name="validation")

                else:
                    win_loss=viz.line(X=np.arange(epoch+1),Y=loss_dict["train"][:epoch+1],win="loss",name="train",opts=dict(title='loss'))
                    win_iou=viz.line(X=np.arange(epoch+1),Y=iou_dict["train"][:epoch+1].mean(1),win="iou",name="train",opts=dict(title='iou'))
                    ...
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1].mean(1),win="iou",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,0],win="car",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,1],win="signal",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,2],win="pedestrian",name="validation")
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1,3],win="lane",name="validation")
            ...
            if phase == "validation" and epoch_iou.mean() > 0.65:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), os.path.join(save_path,"{}_{:.4f}_{:.4f}.pth".format(epoch,epoch_loss,epoch_iou.mean())))
                #print("complete")
        print()
...
if __name__=="__main__":
    ...
    parser.add_argument('-s',dest='seed',type=int,default=ip)
    ...
    save_path=os.path.join(os.path.join(current,"result/ip_"+str(ip)),str(seed))
    ...
    model=model_pytorch.DilatedUNet(in_channels=3,classes=5)
    ...
    model=train_model(model,optimizer,dataloaders=dataloaders,num_epochs=300)
