...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out += self.downsample(x) # (shape: (batch_size, channels, h, w) if stride == 1, (batch_size, channels, h/2, w/2) if stride == 2)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out += self.downsample(x) # (shape: (batch_size, out_channels, h, w) if stride == 1, (batch_size, out_channels, h/2, w/2) if stride == 2)
...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(torch.load(os.path.join(os.getcwd(),"pretrained_models/resnet/resnet50-19c8e357.pth")))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load(os.path.join(os.getcwd(),"pretrained_models/resnet/resnet101-5d3b4d8f.pth")))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load(os.path.join(os.getcwd(),"pretrained_models/resnet/resnet152-b121ed2d.pth")))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
