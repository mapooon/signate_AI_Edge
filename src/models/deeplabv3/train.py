    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', "validation"]:
            ...
            for step,data in enumerate(tqdm(dataloaders[phase])):#tqdm(dataloaders[phase]):
                ...
                loss=CrossEntropyLoss2d()(outputs, labels.argmax(1))

                if phase == 'train':
                    loss.backward()
                    optimizer.step()
                running_loss += loss.data#* batch_size
                running_iou += iou(outputs,labels).data#*batch_size
...
if __name__=="__main__":
    ...
    model=train_model(model,optimizer,dataloaders=dataloaders,num_epochs=2000)