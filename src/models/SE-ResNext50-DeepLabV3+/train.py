...
def train_model(model,optimizer,dataloaders, num_epochs=25):
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[100,160], gamma=0.1)
    #scheduler=lr_scheduler.ReduceLROnPlateau(optimizer,factor=0.1,patience=50)
    #scheduler = lr_scheduler.ReduceLROnPlateau(optimizer,mode='max',factor=0.1,patience=30,min_lr=0.00001)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', "validation"]:
            if phase == 'train':
                #scheduler.step(metrics=iou_dict[phase][epoch-1].mean() if epoch>0 else 0)
                scheduler.step()
            ...
            if phase=="validation":
                ...
                if epoch>0:
                    viz.line(X=np.arange(epoch+1),Y=loss_dict["train"][:epoch+1],update="replace",win="loss",name="train",opts=dict(title='loss'))
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["train"][:epoch+1].mean(1),update="replace",win="iou",name="train",opts=dict(title='iou'))
                    ...
                    viz.line(X=np.arange(epoch+1),Y=loss_dict["validation"][:epoch+1],update="replace",win="loss",name="validation",opts=dict(title='loss'))
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1].mean(1),update="replace",win="iou",name="validation",opts=dict(title='iou'))
                    ...
                    viz.line(X=np.arange(epoch+1),Y=loss_dict["validation"][:epoch+1],win="loss",name="validation",opts=dict(title='loss'))
                    viz.line(X=np.arange(epoch+1),Y=iou_dict["validation"][:epoch+1].mean(1),win="iou",name="validation",opts=dict(title='iou'))
...
if __name__=="__main__":
    ...
    if parsed.use_resize:
        save_path=os.path.join(os.path.join(current,"result/s10241024/ip_"+str(ip)),str(seed))
    else:
        save_path=os.path.join(os.path.join(current,"result/clahe/ip_"+str(ip)),str(seed))
    ...
    if parsed.use_resize:
        data_transforms = {
            'train': transforms.Compose([
                ...
                my_transform.Resize(size=(1024,1024)),
    ...
    model=train_model(model,optimizer,dataloaders=dataloaders,num_epochs=200)
